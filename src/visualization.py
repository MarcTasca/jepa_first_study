...
def visualize_latent_reconstruction(encoder, decoder, save_path='reconstruction.png', mode='circle', 
                                    spiral_loops=3, extra_loops=2, num_points=1000, history_length=3):
    """
    ...
    if np.any(mask_out):
        plt.scatter(reconstructed[mask_out, 0], reconstructed[mask_out, 1], c='orange', s=2, label='Reconstruction (Extrapolation)')
...
def visualize_forecast(encoder, predictor, decoder, save_path='forecast.mp4', num_points=300, dt=0.05, history_length=3):
    """
    ...
    def update(frame):
        ...
        if len(hist_gt_x) > 50: hist_gt_x.pop(0); hist_gt_y.pop(0)
        ...
        if len(hist_pred_x) > 50: hist_pred_x.pop(0); hist_pred_y.pop(0)
